---
title: "Is There a Correlation Between Different Licensed Business Categories That Share One Postal Code?"
...
Our data source is the "Municipal Licensing and Standards - Business Licenses and Permits" dataset, collected by Toronto's Municipal Licensing & Standards (ML&S) and available on the Toronto Open Data Portal [@citedata].

The project had made use of R [@citeR], and R packages "tidyverse" [@citetidyverse], "dplyr" [@citedplyr], "janitor" [@citejanitor], "knitr" [@citeknitr], "stringi" [@stringi], "readr" [@readr], "lubridate" [@lubridate], "purrr" [@purrr], "testthat" [@testthat], and "assertthat" [@assertthat].
...
## Figure 1: Categories with the Highest Maximum Number of Licenses in a Single Postal Code

```{r max, fig.cap="Top 10 Categories with the Highest Maximum Number of Licenses\nin a Single Postal Code", echo=FALSE, message=FALSE}
...
## Figure 2: Categories with the Highest Average Number of Licenses

```{r average, echo=FALSE, fig.cap="Top 10 Categories with the Highest Average Number of Licenses,\nIgnoring Postal Codes with No License of That Particular Category", message=FALSE}
...
## Figure 3: Categories with the Highest Total Number of Licenses

```{r total, fig.cap="Top 10 Categories with the Highest Total Number of Licenses\namong All Postal Codes", echo=FALSE, message=FALSE}
...
analysis_data <- analysis_data %>%
  mutate(`correlation_coefficient` = round(`correlation_coefficient`, 2))
...
analysis_data %>%
  ...
  kable(caption = "Top 10 Highest Correlation Coefficients",
        ...
        linesep = "", 
        digits = c(0, 0, 2),     # Adjusted to 2 decimal places
        booktabs = TRUE) %>%
...
```{r bottom10, echo=FALSE, message=FALSE}
...
```{r density, fig.cap="Density Plot of Correlation Coefficients", echo=FALSE, message=FALSE, fig.height=3}
...
```{r, echo=FALSE, message=FALSE}
summary_stats_df <- read_csv(here::here("outputs/data/statistical_summary.csv"))

# Split the data frame into two parts: one containing the first set of columns and the other the rest
first_half <- summary_stats_df[, 1:8]
second_half <- summary_stats_df[, c(1, 9:ncol(summary_stats_df))]

# Create the first table using kable
kable(first_half, caption = "Statistical Summary of Correlation Coefficients (Part 1)")

# Create a little space between the two tables
cat("\\vspace{5mm}")

# Create the second table using kable
kable(second_half, caption = "Statistical Summary of Correlation Coefficients (Part 2)")
